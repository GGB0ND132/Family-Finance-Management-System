...
csv_content = open("test_clean.csv", encoding="utf-8").read()
...
print("预览结果:", json.dumps(resp, indent=2, ensure_ascii=False))
batch_id = resp["data"]["batch_id"]

# 确认导入
req = urllib.request.Request(
    f"{BASE}/imports/{batch_id}/confirm", data='{}'.encode(),
    headers={"Content-Type": "application/json", "Authorization": f"Bearer {TOKEN}"},
    method="POST",
)
resp = json.loads(urllib.request.urlopen(req).read().decode())
print("确认结果:", json.dumps(resp, indent=2, ensure_ascii=False))
...
resp = json.loads(urllib.request.urlopen(req).read().decode())
print("最终余额:", json.dumps(resp, indent=2, ensure_ascii=False))