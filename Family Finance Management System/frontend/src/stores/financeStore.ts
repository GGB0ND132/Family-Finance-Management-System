...
interface FinanceState {
  ...
  removeOrCloseAccount: (id: string) => 'DELETED' | 'CLOSED' | 'BLOCKED'
...
export const useFinanceStore = create<FinanceState>()(persist((set, get) => ({
  ...
  removeOrCloseAccount: (id) => { const account = get().accounts.find((item) => item.id === id); if (!account || account.currentBalance !== 0) return 'BLOCKED'; const hasReferences = get().transactions.some((item) => item.accountId === id) || get().transfers.some((item) => item.fromAccountId === id || item.toAccountId === id); if (hasReferences) { set((state) => ({ accounts: state.accounts.map((item) => item.id === id ? { ...item, closedAt: now() } : item) })); return 'CLOSED' } set((state) => ({ accounts: state.accounts.filter((item) => item.id !== id) })); return 'DELETED' },
